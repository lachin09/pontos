...
begin
  ...
  -- Serialize retries with the same key so stock is only decremented once.
  ...
  -- Lock all referenced variants in a stable order before checking stock.
  perform v.id
  ...
  select v_order_id, p.id, v.id, p.name, null,
...
revoke all on function public.create_order_secure(uuid, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.create_order_secure(uuid, jsonb, jsonb) to service_role;
