create or replace function public.save_product_with_variants_internal(
  p_product_id uuid,
  p_product jsonb,
  p_variants jsonb
)
...
begin
    ...
    if v_variant_id is null then
      ...
      where product_id = v_product_id
        and (
          ...
          or (size = trim(v_variant->>'size') and color = trim(v_variant->>'color'))
        )
      limit 1;
    end if;
  ...
  where v.product_id = v_product_id
    and not exists (
      ...
      where (nullif(item->>'id', '') is not null and (item->>'id')::uuid = v.id)
        or item->>'sku' = v.sku
        or (item->>'size' = v.size and item->>'color' = v.color)
    );
