alter table public.orders
  add column delivery_country_code text not null default 'UA',
  add column delivery_postal_code text,
  add column nova_poshta_division_id bigint,
  add column nova_poshta_division_name text,
  add column nova_poshta_division_category text;

alter table public.orders
  add constraint orders_delivery_country_code_check check (delivery_country_code ~ '^[A-Z]{2}$'),
  add constraint orders_nova_poshta_division_pair_check check (
    (nova_poshta_division_id is null and nova_poshta_division_name is null)
    or (nova_poshta_division_id is not null and nova_poshta_division_name is not null)
  ),
  add constraint orders_nova_poshta_division_category_check check (
    nova_poshta_division_category is null or nova_poshta_division_category in ('PostBranch', 'CargoBranch', 'Postomat', 'PUDO')
  );
...
begin
  ...
  if (p_customer->>'payment_method') not in ('bank_transfer', 'cash_on_delivery')
    ...
    or ((p_customer->>'nova_poshta_division_id') is not null and (p_customer->>'nova_poshta_division_name') is null) then
    raise exception using errcode = '22023', message = 'Invalid order';
  end if;
  ...
  insert into public.orders (
    ...
    nova_poshta_division_category, payment_method, payment_status, status, comment, subtotal, delivery_price, total
  )
  values (
