...
function OrderStatusEditor({ order }: { order: AdminOrderRow }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(result.error ?? "Не вдалося оновити статус.");
        return;
      }
  ...
  return <div className="flex flex-wrap items-center gap-2">
    <label className="sr-only" htmlFor={`order-status-${order.id}`}>Статус замовлення #{order.orderNumber}</label>
    <select id={`order-status-${order.id}`} value={status} onChange={(event) => { setStatus(event.target.value as OrderStatus); setMessage(null); }} className="min-h-10 rounded-[var(--radius-control)] border border-border bg-background px-2 text-sm">
      {ORDER_STATUSES.map((value) => <option key={value} value={value}>{ORDER_STATUS_LABELS[value]}</option>)}
    </select>
    <button type="button" onClick={() => void save()} disabled={busy || status === order.status} className="min-h-10 rounded-[var(--radius-control)] bg-foreground px-3 text-xs font-medium text-background disabled:cursor-not-allowed disabled:opacity-45">
      {busy ? "Збереження…" : "Зберегти"}
    </button>
    {message && <span className="w-full text-xs text-muted" role="status">{message}</span>}
  </div>;
}
