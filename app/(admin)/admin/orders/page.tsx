...
export default async function AdminOrdersPage() {
  ...
  const { data, error } = await supabase.from("orders").select("id, order_number, first_name, last_name, phone, city, total, status, payment_status, created_at, order_items(id)").order("created_at", { ascending: false }).limit(500);
  return <>
